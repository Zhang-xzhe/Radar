...
%   http://www.isr.uc.pt/~henriques/
	...
	ground_truth = textscan(f, '%f,%f,%f,%f');  %[x, y, width, height]，读取txt文件
	ground_truth = cat(2, ground_truth{:});%cat：用来联结数组
	fclose(f);
	
	%set initial position and size
	target_sz = [ground_truth(1,4), ground_truth(1,3)];
	pos = [ground_truth(1,2), ground_truth(1,1)] + floor(target_sz/2);%floor向下取整
	...
	if ~isempty(text_files),
		...
		fclose(f);
        [video_path num2str(frames{1}, 'imgs/%04i.jpg')]
		%see if they are in the 'imgs' subfolder or not
		if exist([video_path num2str(frames{1}, 'imgs/%04i.jpg')], 'file'),
		...
		if isempty(img_files),
			...
			assert(~isempty(img_files), 'No image files to load.')%不成立则输出警告
		end
	...
	%much detail
	if sqrt(prod(target_sz)) >= 100,%prod计算数组元素的连乘积
		pos = floor(pos / 2);
